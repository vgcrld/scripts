...
#
# ./vmrun Sirius_PSC 1 10    # Run files 1-10 from the ls of the rerun dir
# ./vmrun Sirius_PSC 21 80   # Run files 21-101 from the ls of the rerun dir
#
...
for i in `ls ${rerun}/${file_prefix}*.vmware.gz | lines $start_count $end_count`
do 
  printf "Processing: ${i} "
  if OUTPUT=${input} test-single-file vmware $i > /dev/null 2>&1
  then 
    echo SUCCESS
    mv $i "${finished}"
  else 
