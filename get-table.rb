...
def get_stuff(opts)
  outfile = opts[:file]
  url = opts[:url]
  uri = URI.escape("/#{opts[:site]}/table/nil/ServerAssetsTable.csv?dao=ServerAssetsTable&t=#{opts[:token]}")
  puts url + uri
  exit
  begin
    File.delete(outfile) if File.exist?(outfile)
    http = get_http(url)
    res  = http.get("/#{opts[:site]}/table/nil/ServerAssetsTable.csv?dao=ServerAssetsTable&t=#{opts[:token]}")
    if res.code == "200"
      File.open(outfile, 'w') { |file| file.write(res.body) }
      puts "Created: #{outfile}"
    else
...
SUB_COMMANDS = %w[do_stuff download upload delete_file_tags delete_online_groups delete_online_tags]
